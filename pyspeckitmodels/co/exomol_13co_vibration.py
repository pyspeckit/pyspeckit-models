...
levels_fn = os.path.join(path, '13co_levels.ecsv')
if os.path.exists(levels_fn):
    levels = Table.read(levels_fn)
else:
    resp = requests.get('https://exomol.com/db/CO/13C-16O/Li2015/13C-16O__Li2015.states.bz2')
...
transitions_fn = os.path.join(path, '13co_transitions.ecsv')
if os.path.exists(transitions_fn):
    transitions = Table.read(transitions_fn)
else:
    resp = requests.get('https://exomol.com/db/CO/13C-16O/Li2015/13C-16O__Li2015.trans.bz2')
    resp.raise_for_status
    tablefile = bz2.BZ2File(BytesIO(resp.content))
...
transitions_enhanced_fn = os.path.join(path, '13co_transitions_enhanced.ecsv')
...
partfunc_fn = os.path.join(path, '13co_partfunc.ecsv')
if os.path.exists(partfunc_fn):
    partfunc = Table.read(partfunc_fn)
else:
    resp = requests.get('https://exomol.com/db/CO/13C-16O/Li2015/13C-16O__Li2015.pf')
    resp.raise_for_status()
...
def tau_of_N(wavelength, column, tex=10*u.K, width=1.0*u.km/u.s,
    ...
    if isotopomer != 1316:
        raise NotImplementedError("Partition function only implemented for 13C-16O")
    partition_function = np.interp(tex.to(u.K).value, partfunc['temperature'], partfunc['Q'])
    # print(len(trans))
...
def test():
    from exomol_co_vibration import exomol_xsec
    numin, numax, dnu = 3.815033, 3.895033, 0.01
    tex = 300
    wavelengths = np.arange(numin, numax, dnu)*u.cm**-1

    sigmas = exomol_xsec(numin, numax, dnu, tex, molecule='13C-16O')

    column = 1e15*u.cm**-2
    tex = tex*u.K
    width = np.sqrt(constants.k_B * tex / (29*u.Da)).to(u.km/u.s)
...
def test_vs_pyspeckitmodel():
    ...
    T = 100
    width = ((constants.k_B * T*u.K / (29*u.Da))**0.5).to(u.km/u.s)
    column = 1e18
    L, = pl.plot(xarr,
